...
class Game:
    def __init__(self):
        ...
        self.languages = {
            "English": ["Solo Play", "Multiplayer", "Easy", "Normal", "Hard", "Impossible", "Settings", "Back", "Music: On", "Music: Off", "Language: English", "Speed: Normal", "Speed: Fast"],
            "Français": ["Jeu Solo", "Multijoueur", "Facile", "Normal", "Difficile", "Impossible", "Paramètres", "Retour", "Musique : Activée", "Musique : Désactivée", "Langue : Français", "Vitesse : Normal", "Vitesse : Rapide"],
            "Русский": ["Одиночная игра", "Многопользовательская", "Легко", "Нормально", "Трудно", "Невозможно", "Настройки", "Назад", "Музыка: Вкл", "Музыка: Выкл", "Язык: Русский", "Скорость: Нормально", "Скорость: Быстро"],
            "עברית": ["משחק בודד", "רשת משחקים", "קל", "נורמלי", "קשה", "בלתי אפשרי", "הגדרות", "חזור", "מוזיקה: פועל", "מוזיקה: כבוי", "שפה: עברית", "מהירות: רגילה", "מהירות: מהירה"],
            "العربية": ["لعب فردي", "متعدد اللاعبين", "سهل", "عادي", "صعب", "مستحيل", "الإعدادات", "عودة", "الموسيقى: تشغيل", "الموسيقى: إيقاف", "اللغة: العربية", "السرعة: عادي", "السرعة: سريع"],
            "हिन्दी": ["एकल खेल", "बहुप्रयोक्ता", "आसान", "सामान्य", "कठिन", "असंभव", "सेटिंग्स", "वापस", "संगीत: चालू", "संगीत: बंद", "भाषा: हिन्दी", "गति: सामान्य", "गति: तेज"],
            "中文": ["单人游戏", "多人游戏", "简单", "普通", "困难", "不可能", "设置", "返回", "音乐：开启", "音乐：关闭", "语言：中文", "速度：正常", "速度：快"]
        }
    ...
    def settings_menu(self):
        while True:
            ...
            speed_normal_text = self.languages[language_keys[self.language_index]][11]
            speed_fast_text = self.languages[language_keys[self.language_index]][12]

            back = self.font.render(back_text, True, "white")
            back_rect = back.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 - 100))
            ...
            language_rect = language.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 50))
            speed = self.font.render(speed_normal_text if not self.fast_speed else speed_fast_text, True, "white")
            speed_rect = speed.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 125))
    ...
    def toggle_speed(self):
        self.fast_speed = not self.fast_speed
        if self.fast_speed:
            print("Speed: Fast")
        else:
            print("Speed: Normal")
    ...
    def run(self, mode):
        ...
        while True:
            ...
            if game_outcome:
                ...
                break
            dt = self.clock.tick(60 if not self.fast_speed else 120) / 1000
